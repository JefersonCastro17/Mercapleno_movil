...
class _InventoryPageState extends State<InventoryPage> {
  ...
  Widget _buildProductImage(String imagePath) {
    ...
    final cleanPath = imagePath.startsWith('uploads/')
        ...
        : imagePath;
    final fullUrl = '${AppConfig.apiBaseUrl}/uploads/$cleanPath';
  ...
  Widget _buildProductCard(BuildContext context, Product p, int threshold) {
    ...
    // `imagen` no existe en la entidad Product → se muestra ícono por defecto
    const imageUrl = '';
...
class _MovementFormWrapper extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text(type == 'in' ? 'Registrar Entrada' : 'Registrar Salida'),
        backgroundColor:
            type == 'in' ? const Color(0xFF0B4A8B) : Colors.redAccent,
        foregroundColor: Colors.white,
      ),
      body: SafeArea(
        ...
        children: [
          ...
          TextFormField(
            ...
            validator: (value) {
              ...
              if (widget.type == 'out' && q > _currentStock) {
          ...
          _isSaving
              ...
              : FilledButton(
                  style: FilledButton.styleFrom(
                    backgroundColor: widget.type == 'in'
                        ? const Color(0xFF0B4A8B)
                        : Colors.redAccent,
                  ),
                  onPressed: _submit,
                  child: Text(widget.type == 'in'
                      ? 'Registrar Entrada'
                      : 'Registrar Salida'),
                ),
        ],
